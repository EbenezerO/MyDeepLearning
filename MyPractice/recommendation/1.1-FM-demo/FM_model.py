...
def vectorize_dic(dic,ix=None,p=None,n=0,g=0):#构建稀疏矩阵
    """
    ...
    for k,lis in dic.items():
        for t in range(len(lis)):
            ix[str(lis[t]) + str(k)] = ix.get(str(lis[t]) + str(k),0) + 1#统计用户、物品出现次数
...
train = pd.read_csv('data/ua.base',encoding='latin-1', engine='python',delimiter='\t',names = cols)
test = pd.read_csv('data/ua.test',encoding='latin-1',engine='python',delimiter='\t',names = cols)

x_train,ix = vectorize_dic({'users':train['user'].values, #dic 模式为 key：value，且key唯一  这里为user ：[数组] 间接完成了一对多
